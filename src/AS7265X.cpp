...
void AS7265X::setMeasurementMode(byte mode)
{
	...
	//Read, mask/set, write
	byte value = virtualReadRegister(AS7265X_CONTROL_SETUP); //Read
	value &= 0b11110011; //Clear BANK bits
	value |= (mode << 2); //Set BANK bits with user's choice
	virtualWriteRegister(AS7265X_CONTROL_SETUP, value); //Write
}
...
{
	virtualWriteRegister(AS7265X_INT_T, integrationValue); //Write
}
...
int AS7265X::getChannel(byte channelRegister)
{
...
boolean AS7265X::clearDataAvailable()
{
	byte value = virtualReadRegister(AS7265X_CONTROL_SETUP);
	value &= ~(1 << 1); //Set the DATA_RDY bit
	virtualWriteRegister(AS7265X_CONTROL_SETUP, value);
}
...
void AS7265X::enableBulbAS72653()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72653_LED_CONTROL_UV);
	value |= (1 << 3); //Set the bit
	virtualWriteRegister(AS72653_LED_CONTROL_UV, value);
}

//Disable the onboard 5700k or external incandescent bulb
void AS7265X::disableBulbAS72653()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72653_LED_CONTROL_UV);
...
//Current 2: 50mA
//Current 3: 100mA
void AS7265X::setBulbCurrentAS72653(byte current)
{
	if (current > 0b11) current = 0b11; //Limit to two bits

										//Read, mask/set, write
	byte value = virtualReadRegister(AS72653_LED_CONTROL_UV); //Read
	value &= 0b11001111; //Clear ICL_DRV bits
	value |= (current << 4); //Set ICL_DRV bits with user's choice
	virtualWriteRegister(AS72653_LED_CONTROL_UV, value); //Write
}

//Enable the onboard 5700k or external incandescent bulb
void AS7265X::enableBulbAS72652()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72652_LED_CONTROL_VISIBLE);
	value |= (1 << 3); //Set the bit
	virtualWriteRegister(AS72652_LED_CONTROL_VISIBLE, value);
}

//Disable the onboard 5700k or external incandescent bulb
void AS7265X::disableBulbAS72652()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72652_LED_CONTROL_VISIBLE);
	value &= ~(1 << 3); //Clear the bit
...
//Current 2: 50mA
//Current 3: 100mA
void AS7265X::setBulbCurrentAS72652(byte current)
{
	if (current > 0b11) current = 0b11; //Limit to two bits

										//Read, mask/set, write
	byte value = virtualReadRegister(AS72652_LED_CONTROL_VISIBLE); //Read
	value &= 0b11001111; //Clear ICL_DRV bits
	value |= (current << 4); //Set ICL_DRV bits with user's choice
	virtualWriteRegister(AS72652_LED_CONTROL_VISIBLE, value); //Write
}
...
//Enable the onboard 5700k or external incandescent bulb
void AS7265X::enableBulbAS72651()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72651_LED_CONTROL_NIR);
	value |= (1 << 3); //Set the bit
	virtualWriteRegister(AS72651_LED_CONTROL_NIR, value);
}

//Disable the onboard 5700k or external incandescent bulb
void AS7265X::disableBulbAS72651()
{
	//Read, mask/set, write
	byte value = virtualReadRegister(AS72651_LED_CONTROL_NIR);
	value &= ~(1 << 3); //Clear the bit
...
//Current 2: 50mA
//Current 3: 100mA
void AS7265X::setBulbCurrentAS72651(byte current)
{
	if (current > 0b11) current = 0b11; //Limit to two bits

										//Read, mask/set, write
	byte value = virtualReadRegister(AS72651_LED_CONTROL_NIR); //Read
	value &= 0b11001111; //Clear ICL_DRV bits
	value |= (current << 4); //Set ICL_DRV bits with user's choice
	virtualWriteRegister(AS72651_LED_CONTROL_NIR, value); //Write
}
...
byte AS7265X::getTemperatureAS72651()
{
...
//Returns the temperature in C
//Pretty inaccurate: +/-8.5C
byte AS7265X::getTemperatureAS72652()
{
	return (virtualReadRegister(AS72652_DEVICE_TEMP));
}

//Returns the temperature in C
//Pretty inaccurate: +/-8.5C
byte AS7265X::getTemperatureAS72653()
{
